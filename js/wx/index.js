...
function get_funnel() {
    ...
    $(".load-shadow").show();
    if (funnelDateType == 4) {
        if (localStorage.getItem(xsyUser.id) && localStorage.getItem(xsyUser.id) != "undefined") {
    ...
    function set_funnel() {
        $.ajax({
            ...
            success: function(oData) {
                $("#performancePage .load-shadow").hide();
                // console.log('漏斗：' + oData);
                // var oData = eval('(' + data + ')');
