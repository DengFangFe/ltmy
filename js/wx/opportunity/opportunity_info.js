...
function more() {
    ...
    if ($('#area').length > 0) {
        ...
        $('#relevance_contact').bind('click', function() {
            $('#area').remove();
            relevance_contact();
        });
        $('#set_opportunity').bind('click', function() {
            $('#area').remove();
        ...
        $('#delete_opportunity').bind('click', function() {
            $('#area').remove();
            delete_dialog();
        });
    }
...
function build_contact_list($parent_obj, data, pagesize) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        oList.find('.tel').bind('click', function(e) {
            ...
            location.href = apppath + '/wx/activityrecord/create.action?' + ddcommparams + '&belongId=2&objectId=' + $(this).attr('value') + '&activityTypeId=' + activeRecordTypeOje["TEL"] + '&from=opp_info&opportunityId=' + GetQueryString('opportunityId');
            e.stopPropagation();
        })
        $parent_obj.append(oList);
    }
