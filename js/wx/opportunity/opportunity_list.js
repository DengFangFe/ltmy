...
function add_opportunity() {
    location.href = apppath + '/wx/opportunity/add.action?' + ddcommparams + '&from=opp_list';
}
...
function search() {
    show_history();
    //搜索框键入信息
    $('.input_area #txt').bind('input', function() {
        input_delete();
        val = $('.input_area #txt').val();
        if (val.substr(0, 1) == ' ') {
            val = val.trim();
        }
        if (val == '') {
...
function create_word_list() {
    ...
    build_common_list(apppath + '/wx/opportunity/dolist.action', $('#search_content .main'), '无相关销售机会', build_word_unit, wordScrollFunc);
}
//搜索单元创建
